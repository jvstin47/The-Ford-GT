...
export default function Navigation() {
  ...
  return (
    <>
      ...
      >
        <Link to={ROUTES.HOME} className="font-display text-xl md:text-2xl font-bold tracking-widest text-white uppercase hover:scale-105 transition-transform focus-visible:ring-2 focus-visible:ring-off-white outline-none rounded">
          FORD GT
        </Link>
        ...
        {/* Mobile Hamburger */}
        <button 
          className="md:hidden text-off-white focus-visible:ring-2 focus-visible:ring-off-white outline-none rounded flex items-center p-1"
      ...
      {mobileMenuOpen && (
        <div 
          ref={mobileMenuRef}
          className="fixed inset-0 z-[95] bg-matte-black/95 backdrop-blur-lg flex flex-col items-center justify-center gap-12 animate-fade-in md:hidden"
        >
          <Link to={ROUTES.HERITAGE} className={`font-display text-2xl font-bold tracking-[0.1em] focus-visible:ring-2 focus-visible:ring-off-white outline-none rounded px-4 py-2 ${getLinkStyle(ROUTES.HERITAGE)}`}>HERITAGE</Link>
          <Link to={ROUTES.SPECS} className={`font-display text-2xl font-bold tracking-[0.1em] focus-visible:ring-2 focus-visible:ring-off-white outline-none rounded px-4 py-2 ${getLinkStyle(ROUTES.SPECS)}`}>SPECS</Link>
          <Link to={ROUTES.OWNERSHIP} className={`font-display text-2xl font-bold tracking-[0.1em] focus-visible:ring-2 focus-visible:ring-off-white outline-none rounded px-4 py-2 ${getLinkStyle(ROUTES.OWNERSHIP)}`}>OWNERSHIP</Link>
        </div>
      )}
    </>
  );
